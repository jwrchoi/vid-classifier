"""
Fine-tune Social Distance classifier.

Thin wrapper around trainer.train_model() for standalone single-model training.

Usage:
    python model_training/fine_tune_distance.py [--epochs 30] [--lr 1e-4]
"""
...
def main():
    parser = argparse.ArgumentParser(description="Fine-tune distance classifier")
    ...
    train_ds = FrameDataset(train_df, "distance", cfg["classes"], video_lookup, get_train_transform())
    val_ds = FrameDataset(val_df, "distance", cfg["classes"], video_lookup, get_eval_transform())
    test_ds = FrameDataset(test_df, "distance", cfg["classes"], video_lookup, get_eval_transform())
    ...
    out = Path(args.output) if args.output else Path("distance_resnet50_finetuned.pth")
